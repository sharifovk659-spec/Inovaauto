...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postAction === 'toggle_fav') {
        ...
        $qs = ia_input_safe_query_string($_POST['return_qs'] ?? '');
...
$brandId = ia_get_int('brand_id');
$modelId = ia_get_int('model_id');
$brandName = '';
$modelName = '';
if ($brandId > 0) {
...
$catalogSearchQ = ia_get_search('q');
$catalogPriceMin = ia_get_text('price_min', 20);
$catalogPriceMax = ia_get_text('price_max', 20);
$catalogYear = ia_get_text('year', 4);
$catalogMileageMax = ia_get_text('mileage_max', 12);
$catalogFuelType = ia_input_enum($_GET['fuel_type'] ?? '', ['petrol', 'diesel', 'gas', 'hybrid', 'electric']);
$catalogTransmission = ia_input_enum($_GET['transmission'] ?? '', ['auto', 'manual', 'robot', 'cvt']);
$catalogBodyType = ia_input_enum($_GET['body_type'] ?? '', array_keys(ia_listing_body_types_map()));
$catalogAvailability = ia_input_enum($_GET['availability'] ?? '', ['on_order', 'in_stock']);
$catalogSort = ia_input_enum($_GET['sort'] ?? 'new', ['new', 'price_asc', 'price_desc', 'mileage_asc'], 'new');
$catalogCityFilter = ia_tj_city_normalize(ia_get_text('city', 80));

$data = ia_pub_listings_catalog($pdo, [
    'q' => $catalogSearchQ,
    'brand' => $brandName,
    'model' => $modelName,
    'price_min' => $catalogPriceMin,
    'price_max' => $catalogPriceMax,
    'year' => $catalogYear,
    'mileage_max' => $catalogMileageMax,
    'fuel_type' => $catalogFuelType,
    'transmission' => $catalogTransmission,
    'city' => $catalogCityFilter,
    'body_type' => $catalogBodyType,
            ...
            <div class="ia-catalog-quick-bar">
                ...
                <div class="ia-catalog-quick-field ia-catalog-quick-field--price">
                    ...
                    <div class="ia-catalog-price-range">
                        <input type="number" name="price_min" class="form-control form-control-sm ia-catalog-quick-control" value="<?= ia_h($catalogPriceMin) ?>" min="0" step="1000" placeholder="от" aria-label="Цена от">
                        <input type="number" name="price_max" class="form-control form-control-sm ia-catalog-quick-control" value="<?= ia_h($catalogPriceMax) ?>" min="0" step="1000" placeholder="до" aria-label="Цена до">
                    </div>
                </div>
                <button
                ...
                <div class="ia-catalog-extended-inner">
                    <div class="ia-catalog-extended-grid">
                        <div class="ia-catalog-field">
                            ...
                            <input type="number" name="mileage_max" id="catalogMileage" class="form-control form-control-sm" value="<?= ia_h($catalogMileageMax) ?>" min="0" step="1000" placeholder="макс.">
                        </div>
                        <div class="ia-catalog-field">
                            <label class="ia-catalog-label" for="catalogFuel">Топливо</label>
                            <?php $fuel = $catalogFuelType; ?>
                        ...
                        <div class="ia-catalog-field">
                            ...
                            $iaCitySelectValue = $catalogCityFilter;
