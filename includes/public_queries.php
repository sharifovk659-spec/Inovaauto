 ...
 */
/** Сброс кэша брендов/моделей/счётчиков категорий после правок в админке. */
...
function ia_pub_invalidate_body_type_counts_cache(): void
{
    ia_cache_forget('pub_body_type_counts');
}
...
{
    $page = ia_db_sql_limit(ia_input_int($f['page'] ?? 1, 1, 1), 1, 10000);
    $per = ia_db_sql_limit(ia_input_int($f['per'] ?? 12, 12, 6), 6, 48);
    $offset = ia_db_sql_offset(($page - 1) * $per);
    $sort = ia_input_enum($f['sort'] ?? 'new', ['new', 'price_asc', 'price_desc', 'mileage_asc'], 'new');
    $order = match ($sort) {
    ...
    if ($pmin !== null && $pmin !== '') {
        $pminVal = ia_input_decimal($pmin, 0);
        if ($pminVal !== null) {
            $where[] = 'l.price >= :pmin';
            $params['pmin'] = $pminVal;
        }
    ...
    if ($pmax !== null && $pmax !== '') {
        $pmaxVal = ia_input_decimal($pmax, 0);
        if ($pmaxVal !== null) {
            $where[] = 'l.price <= :pmax';
            $params['pmax'] = $pmaxVal;
        }
    ...
    if ($bodyType !== '') {
        ...
        if (in_array($bodyType, $allowedBody, true)) {
    ...
    $runCatalog = static function (array $w, array $p) use ($pdo, $order, $per, $offset): array {
        ...
        $sql = "SELECT l.* FROM ad_listings l WHERE {$sqlWhere} ORDER BY {$order} LIMIT " . ia_db_sql_limit($per) . ' OFFSET ' . ia_db_sql_offset($offset);
        $st = $pdo->prepare($sql);
        $st->execute($p);
...
function ia_pub_listings_search_suggestions(IaPgConnection|IaPdoConnection $pdo, string $q, int $limit = 8): array
{
    ...
    if ($terms !== []) {
        ...
        $sql = "SELECT l.* FROM ad_listings l
                ...
                LIMIT " . ia_db_sql_limit($limit, 4, 12);
        try {
            $st = $pdo->prepare($sql);
            $st->execute($params);
...
function ia_pub_listings_home(IaPgConnection|IaPdoConnection $pdo, int $limit = 12): array
{
    return ia_db_fetch_all_limit(
        $pdo,
        "SELECT * FROM ad_listings WHERE status = 'approved' AND is_vip = 0 ORDER BY created_at DESC",
        [],
        $limit,
        null
    );
...
function ia_pub_listings_latest(IaPgConnection|IaPdoConnection $pdo, int $limit = 12): array
{
    return ia_db_fetch_all_limit(
        $pdo,
        "SELECT * FROM ad_listings WHERE status = 'approved' ORDER BY created_at DESC",
        [],
        $limit,
        null
    );
...
function ia_pub_listings_vip(IaPgConnection|IaPdoConnection $pdo, int $limit = 12): array
{
    return ia_db_fetch_all_limit(
        $pdo,
        "SELECT * FROM ad_listings WHERE status = 'approved' AND is_vip = 1 ORDER BY created_at DESC",
        [],
        $limit,
        null
    );
...
function ia_pub_popular_brands(IaPgConnection|IaPdoConnection $pdo, int $limit = 12): array
{
    ...
    return ia_cache_remember('pub_popular_brands_' . $limit, $ttl, static function () use ($pdo, $limit): array {
        return ia_db_fetch_all_limit(
            $pdo,
            'SELECT b.id, b.name, COUNT(l.id) AS listings_count
            FROM car_brands b
...
function ia_pub_messages_for_thread(IaPgConnection|IaPdoConnection $pdo, int $threadId, int $limit = 200): array
{
    ...
    $limit = ia_db_sql_limit($limit, 1, 500);
